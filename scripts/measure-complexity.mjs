 ...
 */
async function measure(code, filePath) {
...
if (region) {
  ...
  if (rows.length === 0) {
    console.log(`${target}: no functions reported (every function measures 0)`);
  }
  for (const r of rows) {
    const over = r.cc > 15 ? "  <-- over threshold" : "";
    console.log(`${String(r.cc).padStart(4)}  L${String(r.line).padEnd(5)}  ${r.name}${over}`);
  }
  console.log(`\n${rows.length} functions reported`);
}
