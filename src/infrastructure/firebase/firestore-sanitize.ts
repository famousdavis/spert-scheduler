 ...
 *
 * Safe to use with both `setDoc(...)` (no merge) and the
 * `setDoc(..., { mergeFields: [...] })` path used by `doSave()`: with
 * `mergeFields`, each named top-level key is wholesale replaced, so absent
 * sub-keys in a map are dropped on the server without needing
 * `deleteField()` sentinels.
 */
