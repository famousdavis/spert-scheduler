 ...
 * header. Not decoration: `LICENSE` adds six terms under GPL v3 Section 7 —
 * attribution preservation, legal notice preservation, trademark reservation,
 * origin and marking of modified versions, endorsement and promotion,
 * indemnification — and Section 7 requires a source file carrying such terms
 * to either state them or say where they are found. The header's
 * third line is that notice.
 *
 * This repo held 339 of the suite's 341 two-line headers. They were legally
 * equivalent — same words, wrapped onto one line instead of two — so this was
 * consistency work, not a compliance fix. What made it worth doing is that the
 * variant boundary ran through individual FILES, not repos: `scripts/shipgate.mjs`
 * arrived here already three-line, while `scripts/contract-hash.mjs` was
 * two-line and byte-identical to its copy in the landing-page repo. Under a
 * per-repo variant pin both would have needed permanent exemptions, and the
 * next file copied between repos would have recreated the problem.
 */
...
/** Non-vacuity floor — ~90% of the 344 files in scope today. */
const MIN_FILES = 309;
 ...
 *
 * The closure half is load-bearing for `index.html` and `firestore.rules`,
 * which nothing in `npm run build`, `npm test` or `npm run lint` parses. An
 * unclosed `<!--` would swallow the whole document, and a `firestore.rules`
 * header that lost its `//` would pass a words-only check and break the next
 * `firebase deploy`.
 */
