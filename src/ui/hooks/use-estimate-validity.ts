 ...
 * the same message (REASONED): every throw the distribution factory can raise — Triangular or
 * Beta-PERT out of order; T-Normal, LogNormal or Uniform with Min above Max; LogNormal at zero —
 * is also an issue of the strict schema, and no saved issue is held back any more: nothing is
 * saved while a row's three estimate cells are still being typed in, and a triple left out of
 * order is flagged. It was built in v0.69.0 to skip a held-back half-typed row's throw; it is
 * kept, not simplified, in the PR that removed the reason (simplifying it is a filed follow-up).
 */
