...
- **An estimate with no spread gets no suggestion.** When Min, Most Likely and Max are equal, every distribution gives the same single value, so there is nothing to suggest. A freshly added row whose three estimates all start at 1 no longer carries a dot.

- **No suggestion is shown for an estimate that is not yet valid,** such as one with Min above Most Likely: no curve fits it. The row's red highlight still shows what needs fixing.
...
- **Connect AI picks the same way, from the numbers alone.** An activity an AI creates without naming a distribution still gets one automatically: T-Normal, LogNormal or Triangular, with Triangular when Most Likely equals Min or Max. An estimate with no spread gets the scenario's default distribution, the same one “+ Add Activity” uses.
